...
import com.example.mixis.androidprochatapp.Adapter.ListUsersAdapter;
...
public class ListUsersActivity extends AppCompatActivity {
            ...
            public void onClick(View view) {
            ...
            else if (mode.equals(Common.UPDATE_ADD_MODE) && qbChatDialog!= null)
                {
                    if(userAdd.size()>0)
                    {
                        ...
                        SparseBooleanArray chcechItemPositions = lstUsers.getCheckedItemPositions();
                        for(int i=0 ; i <cntChoice;i++)
                        {
                            if(chcechItemPositions.get(i))
                            {
    ...
    private void loadListUserInGroup() {
        ...
        QBRestChatService.getChatDialogById(qbChatDialog.getDialogId()).performAsync(new QBEntityCallback<QBChatDialog>() {
            ...
            public void onSuccess(QBChatDialog qbChatDialog, Bundle bundle) {
                ...
                List<QBUser> listAlreadyInGroup = QBUsersHolder.getInstance().getUsersByIds(occupantsId);
    ...
    private void createGroupChat(SparseBooleanArray checkedItemPositions) {
        ...
        QBRestChatService.createChatDialog(dialog).performAsync(new QBEntityCallback<QBChatDialog>() {
            ...
            public void onSuccess(QBChatDialog qbChatDialog, Bundle bundle) {
                mDialog.dismiss();
                Toast.makeText(getBaseContext(),"Create chat dialog succesfuly",Toast.LENGTH_SHORT).show();
        ...
        for(int i=0 ;i < countChoice;i++)
        {
            if(checkedItemPositions.get(i))
            {
                ...
                QBRestChatService.createChatDialog(dialog).performAsync(new QBEntityCallback<QBChatDialog>() {
                    ...
                    public void onSuccess(QBChatDialog qbChatDialog, Bundle bundle) {
                        mDialog.dismiss();
                        Toast.makeText(getBaseContext(),"Create private chat dialog succesfuly",Toast.LENGTH_SHORT).show();
